...
base_dir = os.path.expanduser('~/Documents/adaptation/exp2_EEG_pupil')
EEG_results_dir = 'results/EEG'
pupil_results_dir = 'results/pupil'
EEG_plots_dir = 'plots/EEG'
pupil_plots_dir = 'plots/pupil'

# set aesthetics for plots
sns.set(style="ticks")
custom_palette = [(.4,.4,.4),(.3,.3,.9)] # these correspond to luminance and S

#%% EEG adapt regression

# Import the wanted csv file
wide_data = pd.read_csv(os.path.join(base_dir,EEG_results_dir,'EEGreg.csv'))
...
plt.savefig(os.path.join(base_dir,EEG_plots_dir,'EEG_reg.jpg'),dpi=300)
plt.show()

#%% EEG difference (after-before)

# Import the wanted csv file
wide_data = pd.read_csv(os.path.join(base_dir,EEG_results_dir,'difference_wide.csv'))
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.savefig(os.path.join(base_dir,EEG_plots_dir,'EEG_diff_within.jpg'),dpi=300)
plt.show()
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.savefig(os.path.join(base_dir,EEG_plots_dir,'EEG_diff_all.jpg'),dpi=300)
plt.show()

#%% pupil adapt regression

# Import the wanted csv file
wide_data = pd.read_csv(os.path.join(base_dir,pupil_results_dir,'pupilReg.csv'))
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.title('Boxplot of Pupil Regression')
plt.xlabel('Adaptation Condition', fontsize = 'x-large')
plt.ylabel('Pupil Regression', fontsize = 'x-large')
plt.savefig(os.path.join(base_dir,pupil_plots_dir,'pupil_reg.jpg'),dpi=300)
plt.show()

#%% difference (after-before)

# Import the wanted csv file
wide_data = pd.read_csv(os.path.join(base_dir,pupil_results_dir,'difference_mean_wide.csv'))
...
# boxplots
# within - this is a useful separate plot for presentations as 9 conditions is a lot in one
subsetWithin = long_data[long_data['Condition'].isin(['Lum / Lum', 'S / S'])]
plt.figure(figsize=(6, 6))
ax = sns.boxplot(data=subsetWithin, x='Condition', y='Score', palette=custom_palette,notch=True,width=.2)
# ** ANOVA annotations **
# post_hoc_results = pg.pairwise_tests(dv='Score', within='Condition', subject='Subject', data=subsetWithin, padjust='fdr_bh')
...
plt.xlabel('Probe', fontsize = 'x-large')
plt.xticks([0,1],['Lum','S'])
plt.ylabel('Pupil Difference (Proportion)', fontsize = 'x-large')
plt.ylim([-1,1])
plt.yticks([-1,-.5,0,.5,1])
plt.axhline(y=0, color='darkgray', linestyle='--', zorder=0)
ax.margins(x=0.2)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.savefig(os.path.join(base_dir,pupil_plots_dir,'pupil_diff_within.jpg'),dpi=300)
plt.show()
...
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
plt.savefig(os.path.join(base_dir,pupil_plots_dir,'pupil_diff_all.jpg'),dpi=300)
plt.show()
