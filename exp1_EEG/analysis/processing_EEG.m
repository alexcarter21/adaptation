...
homeDir = getenv('HOME');   
expDir = ('/Documents/adaptation/exp1_EEG/');
dataDir = strcat(homeDir,expDir,'data/');
resultsDir = strcat(homeDir,expDir,'results/');
plotsDir = strcat(homeDir,expDir,'plots/');
...
% variables
condNames = {'Lum / Lum','Lum / L-M','Lum / S','L-M / Lum','L-M / L-M','L-M / S','S / Lum','S / L-M','S / S'};
...
for thisROI = 1:2
    ...
    for thisFolderIndex = 3:length(dirPath) % could be parfor
        ...
        % **** load in ppt files ****
        % trial order
        trialFiles = dir(fullfile(dirPath{thisFolderIndex}, '/*trialDataAfter.mat'));
        % one ppt has 3 blocks, the rest have 5 so we need to check this for each ppt
        nReps = length(trialFiles);
        trialOrderAll = [];
        for thisTrialIndex = 1:nReps
            thisTrialFile = strcat(trialFiles(thisTrialIndex).folder, '/', trialFiles(thisTrialIndex).name);
            thisTrialData = load(thisTrialFile);
            thisTrialOrder = thisTrialData.R.trialOrderPerBlock;
            trialOrderAll = cat(2,trialOrderAll,thisTrialOrder);
        end
        ...
        preProbe_binned = reshape(preProbe_filtered(:,1:64,:), [nConds*nReps,64,sampleFreqHz,probeDurSecs]);
        adapt_binned = reshape(adapt_filtered(:,1:64,:), [nConds*nReps,64,sampleFreqHz,adaptDurSecs]);
        postProbe_binned = reshape(postProbe_filtered(:,1:64,:), [nConds*nReps,64,sampleFreqHz,probeDurSecs]);
        ...
        % **** sort conditions ****
        % get trial order
        [sortedTrialOrder,sortIndices] = sort(trialOrderAll);
        ...
        preProbe_aveConds = conditionAverage(preProbe_ROI,nReps,nConds);
        adapt_aveConds = conditionAverage(adapt_ROI,nReps,nConds);
        postProbe_aveConds = conditionAverage(postProbe_ROI,nReps,nConds);
...
adapt_perBin_Lum = squeeze(mean(adapt_snrPerBin(1:3,signalFreq,:,:),1));
adapt_perBin_LM = squeeze(mean(adapt_snrPerBin(4:6,signalFreq,:,:),1));
adapt_perBin_S = squeeze(mean(adapt_snrPerBin(7:9,signalFreq,:,:),1));

adapt_perBin_pptAve_Lum = squeeze(nanmean(adapt_perBin_Lum,2));
adapt_perBin_pptAve_LM = squeeze(nanmean(adapt_perBin_LM,2));
...
adapt_perBin_sem_LM = squeeze(nanstd(adapt_perBin_LM,[],2))/sqrt((length(dirPath)-2));
...
hold on
shadedErrorBar(extraStartSecs+1:adaptDurSecs,adapt_perBin_pptAve_LM(extraStartSecs+1:adaptDurSecs),adapt_perBin_sem_LM(extraStartSecs+1:adaptDurSecs),'-r.',1);
...
adapt_perBin_Lum = squeeze(mean(adapt_snrPerBin(1:3,signalFreq,extraStartSecs+1:adaptDurSecs,:),1));
adapt_perBin_LM = squeeze(mean(adapt_snrPerBin(4:6,signalFreq,extraStartSecs+1:adaptDurSecs,:),1));
adapt_perBin_S = squeeze(mean(adapt_snrPerBin(7:9,signalFreq,extraStartSecs+1:adaptDurSecs,:),1));
...
for thisPpt = 1:(length(dirPath)-2)
    ...
    thisPptDataLM = squeeze(adapt_perBin_LM(:,thisPpt));
    thisPptDataS = squeeze(adapt_perBin_S(:,thisPpt));
    
    thisPptRegLum = fitlm(1:adaptDurSecs-extraStartSecs, thisPptDataLum);
    thisPptRegLM = fitlm(1:adaptDurSecs-extraStartSecs, thisPptDataLM);
    ...
    lmSlope = thisPptRegLM.Coefficients{2,:};
    lmRegAllPpts = cat(1, lmRegAllPpts, lmSlope);
    
    sSlope = thisPptRegS.Coefficients{2,:};
    sRegAllPpts = cat(1, sRegAllPpts, sSlope);
end

adapt_regression = cat(2, lumRegAllPpts(:,1), lmRegAllPpts(:,1), sRegAllPpts(:,1));
adaptHeadings = {'Luminance','L-M','S'};
...
figure(4);
boxplot(adapt_regression, 'Notch','on', 'Colors', 'k','Labels', adaptHeadings);
title('REGRESSION');
yline(0,':');
